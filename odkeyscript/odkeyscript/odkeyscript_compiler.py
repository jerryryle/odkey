...
class Lexer:
    ...
    # Modifier mappings
    MODIFIER_MAP = {
        'M_LEFTCTRL': 0x01, 'M_LEFTSHIFT': 0x02, 'M_LEFTALT': 0x04, 'M_LEFTGUI': 0x08,
        'M_RIGHTCTRL': 0x10, 'M_RIGHTSHIFT': 0x20, 'M_RIGHTALT': 0x40, 'M_RIGHTGUI': 0x80
    }
    ...
    def _tokenize(self) -> None:
        ...
        while self.position < len(self.source):
            ...
            if char.isspace():
                ...
                self.tokens.append(Token(TokenType.BRACE_OPEN, char, self.line, self.column))
                self.position += 1
                self.column += 1
            elif char == '}':
                self.tokens.append(Token(TokenType.BRACE_CLOSE, char, self.line, self.column))
                self.position += 1
                self.column += 1
            elif char == '"':
                ...
                raise CompileError(f"Unexpected character: {char}", self.line, self.column)
    ...
    def _tokenize_identifier(self) -> None:
        ...
        if identifier in ['press_time', 'keydn', 'keyup', 'press', 'type', 'repeat', 'pause']:
            self.tokens.append(Token(TokenType.COMMAND, identifier, start_line, start_column))
        elif identifier.startswith('M_'):
            self.tokens.append(Token(TokenType.MODIFIER, identifier, start_line, start_column))
        else:
            self.tokens.append(Token(TokenType.KEY, identifier, start_line, start_column))
...
class Compiler:
    ...
    def __init__(self) -> None:
        ...
        self.loop_stack: List[Tuple[int, int, int]] = []  # Stack of (counter_index, loop_start_address, loop_end_address)
    ...
    def _compile_statement(self, lexer: Lexer) -> None:
        ...
        if token.type == TokenType.COMMAND:
            if token.value == 'press_time':
                ...
                raise CompileError(f"Unknown command: {token.value}", token.line, token.column)
        else:
            raise CompileError(f"Expected command, got {token.type}", token.line, token.column)
    ...
    def _compile_press_time(self, lexer: Lexer) -> None:
        ...
        if not lexer.tokens or lexer.tokens[0].type != TokenType.NUMBER:
            raise CompileError("Expected number after press_time", lexer.tokens[0].line, lexer.tokens[0].column)
        
        time_value = int(lexer.tokens[0].value)
        if time_value < 0 or time_value > 65535:
            raise CompileError("press_time must be between 0 and 65535", lexer.tokens[0].line, lexer.tokens[0].column)
        ...
        while lexer.tokens and lexer.tokens[0].type != TokenType.EOF:
            ...
            if token.type == TokenType.MODIFIER:
                if token.value in Lexer.MODIFIER_MAP:
                    ...
                    raise CompileError(f"Unknown modifier: {token.value}", token.line, token.column)
                lexer.tokens.pop(0)
            elif token.type == TokenType.KEY:
                if token.value in Lexer.KEY_MAP:
                    if len(keys) >= 6:
                        raise CompileError("Too many keys (maximum 6)", token.line, token.column)
                    keys.append(Lexer.KEY_MAP[token.value])
                else:
                    raise CompileError(f"Unknown key: {token.value}", token.line, token.column)
                lexer.tokens.pop(0)
            else:
                break
    ...
    def _compile_keyup(self, lexer: Lexer) -> None:
        ...
        if not lexer.tokens or lexer.tokens[0].type in [TokenType.EOF, TokenType.COMMENT]:
    ...
    def _compile_press(self, lexer: Lexer) -> None:
        ...
        if not keys:
            raise CompileError("press command requires at least one key", lexer.tokens[0].line if lexer.tokens else 0, 0)
    ...
    def _compile_type(self, lexer: Lexer) -> None:
        ...
        if not lexer.tokens or lexer.tokens[0].type != TokenType.STRING:
            raise CompileError("Expected string after type", lexer.tokens[0].line, lexer.tokens[0].column)
    ...
    def _compile_repeat(self, lexer: Lexer) -> None:
        ...
        if not lexer.tokens or lexer.tokens[0].type != TokenType.NUMBER:
            raise CompileError("Expected number after repeat", lexer.tokens[0].line if lexer.tokens else 0, lexer.tokens[0].column if lexer.tokens else 0)
        
        count = int(lexer.tokens[0].value)
        if count < 0 or count > 65535:
            raise CompileError("repeat count must be between 0 and 65535", lexer.tokens[0].line, lexer.tokens[0].column)
        ...
        if next_token.type != TokenType.BRACE_OPEN:
            raise CompileError("Expected '{' after repeat count", next_token.line, next_token.column)
        
        lexer.tokens.pop(0)  # Remove '{'
        
        # Check for nested loop limit
        if len(self.loop_stack) >= self.max_counters:
            raise CompileError(f"Too many nested loops (maximum {self.max_counters})", lexer.tokens[0].line, lexer.tokens[0].column)
        ...
        self.loop_stack.append((counter_index, loop_start, 0))  # loop_end will be filled later
        ...
        if close_token.type != TokenType.BRACE_CLOSE:
            raise CompileError("Expected '}' to close repeat block", close_token.line, close_token.column)
    ...
    def _compile_pause(self, lexer: Lexer) -> None:
        ...
        if not lexer.tokens or lexer.tokens[0].type != TokenType.NUMBER:
            raise CompileError("Expected number after pause", lexer.tokens[0].line, lexer.tokens[0].column)
        
        time_value = int(lexer.tokens[0].value)
        if time_value < 0 or time_value > 65535:
            raise CompileError("pause time must be between 0 and 65535", lexer.tokens[0].line, lexer.tokens[0].column)
